...
function preload() {
    ...
    music = loadSound("music back.mp3");
}
...
function draw() {
    ...
    if (gameState === "serve") {
        ...
        if (home.x < 300) {
            home.x = home.width / 2 + 700;
        }
        if (touches.length > 0 || keyDown("up") && nobita.y >= height / 2 - 100) {
            nobita.velocityY = -12;
            nobita.addImage(flyImg);
            touches = [];
        }
...
function spawnFoods() {
    if (frameCount % 80 === 0) {
        food = createSprite(width + 10, Math.round(random(150, height - 50)), 40, 10);
        food.velocityX = -17;
        food.scale = 0.2;
        ...
        switch (ran) {
            case 1:
                ...
                break;
            case 2:
                food.addImage(cakeImg);
                break;
            case 3:
                food.addImage(pizzaImg);
                break;
            case 4:
                food.addImage(chowImage);
                break;
            default:
                break;
        }
...
function spawnObstacles() {
    ...
    if (frameCount % 140 === 0) {
        var obstacle1 = createSprite(width - 10, height - 100, 10, 40);
        obstacle1.velocityX = -17;
        obstacle1.addImage(gianImg);
        obstacle1.scale = 0.7;
        obstacle1.lifetime = 300;
        restart.depth = obstacle1.depth;
        restart.depth = restart.depth + 1;
        back.depth = restart.depth;
        gameOver.depth = restart.depth;
